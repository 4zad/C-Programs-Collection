...
void getUserLogin(struct UserLogin* userLogin) {
	...
	do {
		...
		for (i = 0, whitespace = 0; userLogin->userName[i] != '\0'; i++) {

			if (isspace(userLogin->userName[i]) > 0) {
				whitespace = 1;
...
void getDemographic(struct Demographic* demographic) {
	...
	for (i = 0; demographic->residenceCountry[i] != '\0'; i++) {
...
void updateUserLogin(struct UserLogin* userLogin) {
	...
	int i, menuSelection, digit, uppercase, lowercase, symbol, flag = 0;
...
//7. Gets input for the password and validates it
void getValidatedPass(struct UserLogin* userLogin) {

	int i, digit, uppercase, lowercase, symbol, flag = 0;
	...
	do {
		...
		for (i = 0, digit = 0, uppercase = 0, lowercase = 0, symbol = 0; userLogin->userPass[i] != '\0'; i++) {

			if (isdigit(userLogin->userPass[i]) > 0) {
				digit++;

			} else if (isupper(userLogin->userPass[i]) > 0) {
				uppercase++;

			} else if (islower(userLogin->userPass[i]) > 0) {
				lowercase++;
