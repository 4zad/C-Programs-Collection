...
char getCharOption(const char validValueList[]) {
	...
	do {
		...
		if (charAmntTwo == '\n') {
			...
			for (i = 0; i < strlen(validValueList); i++) {
...
void getCString(char* stringRecieved, int lowLimit, int highLimit) {
	...
	do {
		...
		if (stringLength <= highLimit && stringLength >= lowLimit) {
			strcpy(stringRecieved, stringInput);
			flag = 1;
...
//10. Ensures int value of 0 or greater is entered
int getZeroPositive(void) {
...
//11. Gets number of digits, uppercase, lowercase, symbols, and whitespace in a string
