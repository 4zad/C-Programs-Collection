...
void menuAgent(struct AccountTicketingData* database, const struct Account* account) {
	...
	int i, menuSelection, indexNmbr, ticketIndex, flag = 0, accNmbr = 0;
	char confirmSelection;
	...
	do {
			...
			if (database->accounts[indexNmbr].accountNmbr == account->accountNmbr) {
				...
				case 'Y':
					...
					for (i = 0; i < database->TICKET_MAX_SIZE; i++) {
						...
						if (database->accounts[indexNmbr].accountNmbr == database->tickets[i].accountNmbr && database->tickets[i].ticketStatus) {

							database->tickets[i].ticketNmbr = 0;
							database->tickets[i].mssgNmbr = 0;
		...
		case 7:
			statusFilteredTicketMenu(database, 1);
			break;

		case 8:
			statusFilteredTicketMenu(database, 0);
			break;

		case 9:
			ticketIndex = findTicketIndexNmbr(1, database, 1);
			putchar('\n');
		...
		case 10:
			printf("Feature #10 currently unavailable!\n\n");
			break;
...
void menuCustomer(struct AccountTicketingData* database, const struct Account* account) {
	...
	int menuSelection, ticketIndex, flag = 0;
	...
	do {
		...
		case 2:
			ticketIndex = findTicketIndexNmbr(0, database, 0);

			if (ticketIndex == -1) {
				printf("ERROR: Ticket listing is FULL, call ITS Support!\n\n");

			} else {
				createTicket(database, account, ticketIndex);
		...
		case 3:
			...
			if (database->tickets[ticketIndex].accountNmbr == account->accountNmbr && database->tickets[ticketIndex].ticketStatus) {
				modifyTicketMenu(&database->tickets[ticketIndex], account);
				pauseExecution();

			} else if (database->tickets[ticketIndex].accountNmbr != account->accountNmbr) {
...
int findTicketIndexNmbr(int ticketNmbr, const struct AccountTicketingData* database, int prompt) {

	int i, ticketIndex = -1;

	if (prompt == 1) {
		printf("Enter ticket number: ");
		ticketNmbr = getPositiveInteger();
...
void ticketMessages(const int* ticketIndex, const struct Ticket* ticketDatabase) {
	...
	printf("================================================================================\n");
	printf("%06d (%s) Re: %s\n", ticketDatabase[*ticketIndex].ticketNmbr, ticketDatabase[*ticketIndex].ticketStatus ? "ACTIVE" : "CLOSED", ticketDatabase[*ticketIndex].subjectLine);
	printf("================================================================================\n");

	for (i = 0; i < NMBR_MSSG_MAX && !flag; i++) {
...
void customerTicketlist(const struct AccountTicketingData* database, const struct Account* account) {
	...
	do {
		...
		for (i = 0; i < database->TICKET_MAX_SIZE; i++) {
			...
			if (account->accountNmbr == database->tickets[i].accountNmbr && database->tickets[i].ticketNmbr != 0)
				printf("%06d %-6s %-30s %5d\n", database->tickets[i].ticketNmbr, database->tickets[i].ticketStatus ? "ACTIVE" : "CLOSED", database->tickets[i].subjectLine, database->tickets[i].mssgNmbr);
		...
		if (ticketNmbr == 0) {
			...
			ticketIndex = findTicketIndexNmbr(ticketNmbr, database, 0);
...
void statusFilteredTicketMenu(const struct AccountTicketingData* database, const int status) {
	...
	do {
		...
		for (i = 0; i < database->TICKET_MAX_SIZE; i++) {
			...
			if (database->tickets[i].ticketStatus == status && database->tickets[i].ticketNmbr != 0)
				printf("%06d %5d %-15s %-6s %-30s %5d\n", database->tickets[i].ticketNmbr, database->tickets[i].accountNmbr, database->tickets[i].message->displayName, database->tickets[i].ticketStatus ? "ACTIVE" : "CLOSED", database->tickets[i].subjectLine, database->tickets[i].mssgNmbr);
...
void newTicketMenu(const struct AccountTicketingData* database) {
	...
	do {
		...
		for (i = 0; i < database->TICKET_MAX_SIZE; i++) {
			...
			if (database->tickets[i].ticketStatus == 1 && database->tickets[i].message[1].accountType == 0 && database->tickets[i].ticketNmbr != 0)
				printf("%06d %5d %-15s %-6s %-30s %5d\n", database->tickets[i].ticketNmbr, database->tickets[i].accountNmbr, database->tickets[i].message->displayName, database->tickets[i].ticketStatus ? "ACTIVE" : "CLOSED", database->tickets[i].subjectLine, database->tickets[i].mssgNmbr);
